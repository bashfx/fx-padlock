#!/usr/bin/env bash
# Generated by build.sh on Sat Aug 16 03:31:27 PM MDT 2025
# Auto-assembled from numbered modules: 01_header.sh 02_config.sh 03_stderr.sh 04_helpers.sh 05_printers.sh 06_api.sh 07_core.sh 08_main.sh
# === 01_header.sh ===
#!/usr/bin/env bash
#
#  ____            _  _            _    
...
__log() {
    ...
    case "$type" in
        ...
        error) __printx "$text\n" "red" "$fail " "$stream" ;;
        warn)  [[ $force -eq 1 || $opt_debug -ne 1 ]] && __printx "$text\n" "orange" "$delta " "$stream" ;;
        okay)  [[ $force -eq 1 || $opt_debug -ne 1 ]] && __printx "$text\n" "green" "$pass " "$stream" ;;
        info)  [[ $opt_debug -ne 1 ]] && __printx "$text\n" "blue" "$recv " "$stream" ;;
